...
import { deployDao, getNetworkDetails } from '../utils/DeploymentUtil';

require("dotenv").config();

// @ts-ignore
export const deployContracts = async (deployer, network, accounts) => {
    let res;
    ...
    if (dao) {
        ...
        console.log(
            `Multicall: ${testContracts.multicall ? testContracts.multicall.address : ""
            }`
        );
        ...
        console.log(
            `ERC20Extension: ${extensions.erc20Ext ? extensions.erc20Ext.address : ""
            }`
        );
        console.log("************************");
    } else {
...
// @ts-ignore
const deployGanacheDao = async (deployFunction, network, accounts, truffleImports) => {
    if (!process.env.DAO_NAME) throw Error("Missing env var: DAO_NAME");
    if (!process.env.ERC20_TOKEN_NAME)
        throw Error("Missing env var: ERC20_TOKEN_NAME");
    if (!process.env.ERC20_TOKEN_SYMBOL)
...
// @ts-ignore
const deployTestDao = async (deployFunction, network, accounts, truffleImports) => {
    if (!process.env.DAO_NAME) throw Error("Missing env var: DAO_NAME");
    if (!process.env.ERC20_TOKEN_NAME)
        throw Error("Missing env var: ERC20_TOKEN_NAME");
    if (!process.env.ERC20_TOKEN_SYMBOL)
